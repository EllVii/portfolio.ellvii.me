...
<div class="lpb-panel" id="lpbPanel" role="dialog" aria-modal="true" aria-labelledby="lpbTitle" style="display:none;">
  ...
  <div class="lpb-resize" id="lpbResize" aria-hidden="true"></div>
    ...
    function post(role, html){
      ...
      el.className = 'lpb-msg ' + (role === 'user' ? 'user' : 'bot');
      el.innerHTML = html;
      $body.appendChild(el);
    ...
    (function(){
      ...
      window.addEventListener('mousemove', e=>{
        ...
        Object.assign($panel.style,{
          left:(sl+dx)+'px', top:(st+dy)+'px', right:'auto', bottom:'auto'
        });
      });
    ...
    $form.addEventListener('submit', async (e)=>{
      ...
      // Optional AI summary (will show friendly error if quota/billing issue)
      try{
        ...
        if (ai.text) {
          ...
          const friendly = /insufficient_quota|missing openai/i.test(ai.error)
